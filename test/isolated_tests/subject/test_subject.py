...
@pytest.mark.parametrize(
    'attr', ['security_manager', 'session_id', 'subject', 'identifier',
...
def test_dsc_resolve_identifier_exists(default_subject_context):
    """
    unit tested:  resolve_identifier

    test case:
    resolves to the dsc's identifier attribute when it is set
    """
    dsc = default_subject_context
    result = dsc.resolve_identifier()
    assert result == dsc.identifier and bool(result)

def test_dsc_resolve_identifier_none_accountreturns(
        default_subject_context, monkeypatch, subject_context):
    """
    unit tested:  resolve_identifier

    test case:
    when the dsc doesn't have an identifier attribute set, but the subject attr
    does, the subject's identifier is returned
    """
    dsc = default_subject_context
...
def test_dsc_resolve_identifier_none_sessionreturns(
        default_subject_context, monkeypatch):
    """
    unit tested:  resolve_identifier

    test case:
    when the dsc doesn't have an identifier attribute set, and neither account
    nor subject has identifier, resolve_session is called to obtain a session,
    and then the session's identifier is obtained
    """
    dsc = default_subject_context
    class DumbSession:
...
def test_ds_identifier_fromstack(delegating_subject, monkeypatch):
    """
    unit tested:  identifier

    test case:
    first tries to obtain identifier from get_run_as_identifier_stack
    """
    ds = delegating_subject
    monkeypatch.setattr(ds, 'get_run_as_identifier_stack', lambda: ['identifier1'] )
    result = ds.identifier
    assert result == 'identifier1'


def test_ds_identifier_fromidentifier(
        delegating_subject, monkeypatch, simple_identifier_collection):
    """
    unit tested:  identifier

    test case:
    first tries to obtain identifier from get_run_as_identifier_stack, fails,
    and reverts to _identifier attribute
    """
    ds = delegating_subject
    monkeypatch.setattr(ds, 'get_run_as_identifier_stack', lambda: None)
    result = ds.identifier
    assert result == simple_identifier_collection


def test_ds_is_permitted_withidentifier(delegating_subject, monkeypatch):
    """
    unit test:  is_permitted

    test case:
    the identifier attribute is set for the fixture and is passed on as an
...
def test_ds_is_permitted_withoutidentifier(delegating_subject, monkeypatch):
    """
    unit test:  is_permitted

    test case:
    when no identifier attribute is set, no permission can be determined and
    so an exception raises
    """
    ds = delegating_subject
    monkeypatch.setattr(ds, 'get_run_as_identifier_stack', lambda: None)
    monkeypatch.setattr(ds, '_identifier', None)
...
def test_ds_is_permitted_collective(delegating_subject):
    ...
    test case:
        given a DS with identifier attribute:
...
def test_ds_assert_authz_check_possible(delegating_subject, monkeypatch):
    ...
    test case:
    raises an exception when identifier attribute isn't set
    """
    ds = delegating_subject
    monkeypatch.setattr(ds, 'get_run_as_identifier_stack', lambda: None)
    monkeypatch.delattr(ds, '_identifier')
...
def test_ds_check_permission_raises(delegating_subject, monkeypatch):
    ...
    test case:
    requires the identifier attribute, raising if it doesn't exist
    """
    ds = delegating_subject
    monkeypatch.setattr(ds, 'assert_authz_check_possible', lambda: None)
    monkeypatch.setattr(ds, 'get_run_as_identifier_stack', lambda: None)
...
def test_ds_has_role_raises(delegating_subject, monkeypatch):
    ...
    test case:
    when the identifier attribute isn't set, an exception is raised
    """
    ds = delegating_subject
    monkeypatch.setattr(ds, 'get_run_as_identifier_stack', lambda: None)
    monkeypatch.setattr(ds, '_identifier', None)
...
def test_has_role_collective(delegating_subject, monkeypatch):
    ...
    test case:
    has identifier and so delegates to security_master
    """
    ds = delegating_subject
...
def test_check_role(delegating_subject):
    ...
    test case:
    when identifier attribute exists, delegates request to
...
def test_ds_login_succeeds(
    ...
    - obtains a mocksubject from security_manager.login, which includes
      identifier and host attributes (patched in), both of which are assigned
      to the DS
    ...
    monkeypatch.setattr(mock_subject, '_identifier', sic)
    monkeypatch.setattr(ds, 'get_run_as_identifier_stack', lambda:  None)
        ...
        with mock.patch.object(MockSecurityManager, 'login') as mock_smlogin:
            ...
            with mock.patch.object(DelegatingSubject, 'decorate') as mock_dec:
                ...
                assert (ds.session == mock_session and
                        ...
                        ds._identifier == simple_identifier_collection)
...
def test_ds_login_noidentifier_raises(
        delegating_subject, monkeypatch, mock_subject):
    """
    unit tested:  login
...
def test_ds_login_nohost(
    ...
    - uses a MockSecurityManager, which is patched to return a MockSubject when
      login is called
    - obtains a mocksubject from security_manager.login, which includes
      identifier but NO host attribute
...
@pytest.mark.parametrize('attr',
                         ['security_manager', 'identifier', 'session',
                          'authenticated'])
...
def test_logout(delegating_subject):
    ...
    with mock.patch.object(DelegatingSubject, 'clear_run_as_identities_internal') as mock_clear:
        ...
        with mock.patch.object(MockSecurityManager, 'logout') as mocksm_logout:
            ...
            assert (ds._session is None and ds._identifier is None and
...
def test_ds_run_as(delegating_subject):
    ...
    test case:
    pushes the current delegating subject's identifier onto the run_as stack
    """
    ds = delegating_subject
    with mock.patch.object(DelegatingSubject, 'push_identity') as mock_pi:
        mock_pi.return_value = None
        ds.run_as('myidentifier')
        mock_pi.assert_called_once_with('myidentifier')
...
def test_ds_run_as_raises(delegating_subject, monkeypatch):
    ...
    test case:
    without the identifier attribute, an exception is raised
    """
    ds = delegating_subject
    monkeypatch.setattr(ds, 'get_run_as_identifier_stack', lambda: None)
    monkeypatch.setattr(ds, '_identifier', None)
    pytest.raises(IllegalStateException, "ds.run_as('dumb_identifier')")


@pytest.mark.parametrize('stack, expected',
                         [(collections.deque(['identifier']), True),
...
def test_ds_is_run_as_accessor(delegating_subject, stack, expected, monkeypatch):
    ...
    test case:
    bools an empty deque or deque with identifier
    """
    ds = delegating_subject
    monkeypatch.setattr(ds, 'get_run_as_identifier_stack', lambda: stack)
    ds.is_run_as == expected

def test_ds_get_previous_identifier_wo_stack(delegating_subject, monkeypatch):
    """
    unit tested:  get_previous_identifier
    ...
    ds = delegating_subject
    monkeypatch.setattr(ds, 'get_run_as_identifier_stack', lambda: None)
    result = ds.get_previous_identifier()
    assert result is None


def test_ds_get_previous_identifier_w_singlestack(delegating_subject, monkeypatch):
    """
    unit tested:  get_previous_identifier

    test case:
    when a single-element stack is obtained, previous_identifier == self.identifier
    """
    ds = delegating_subject
    stack = collections.deque(['one'])
    monkeypatch.setattr(ds, 'get_run_as_identifier_stack', lambda: stack)
    result = ds.get_previous_identifier()
    assert result == ds.identifier


def test_ds_get_previous_identifier_w_multistack(delegating_subject, monkeypatch):
    """
    unit tested:  get_previous_identifier
...
def test_ds_get_run_as_identifier_stack(
        delegating_subject, key, expected, monkeypatch, mock_session):
    """
    unit tested:  get_run_as_identifier_stack
    ...
    result = ds.get_run_as_identifier_stack()
    assert result == expected
...
def test_ds_clear_run_as_identities(
    ...
    with mock.patch.object(MockSession, 'remove_attribute') as mock_ra:
        ...
        mock_ra.assert_called_once_with(ds.run_as_identifier_session_key)
...
def test_ds_push_identity_raises(delegating_subject, monkeypatch):
    ...
    test case:
    when no identifier argument is passed, an exception is raised
    """
    ds = delegating_subject
...
def test_ds_push_identity_withstack(
    ...
    test case:
    adds the identifier argument to the existing stack
    """
    ds = delegating_subject
    sic = simple_identifier_collection
    newstack = collections.deque(['collection1'])
    monkeypatch.setattr(ds, 'get_run_as_identifier_stack', lambda: newstack)
    monkeypatch.setattr(ds, 'get_session', lambda: mock_session)
    ds.push_identity(sic)
    assert (mock_session.session[ds.run_as_identifier_session_key] ==
...
def test_ds_push_identity_withoutstack(
    ...
    test case:
    creates a new stack and adds the identifier argument to it
    """
    ds = delegating_subject
    sic = simple_identifier_collection
    monkeypatch.setattr(ds, 'get_run_as_identifier_stack', lambda: None)
...
def test_ds_pop_identity_withmultistack(
    ...
    result = ds.pop_identity()
    session_stack = mock_session.get_attribute(ds.run_as_identifier_session_key)
...
def test_dss_save_to_session(default_subject_store):
    ...
    test case:
    merges identifier and authentication state
    """
    dss = default_subject_store
    with mock.patch.object(DefaultSubjectStore, 'merge_identifier') as dss_mi:
...
def test_dss_merge_identifier_runas(
        default_subject_store, delegating_subject, monkeypatch):
    """
    unit tested:  merge_identifier

    test case:
    with run_as set for the DS, current_identifier is obtained directly from
    the _identifier attribute (bypassing the property logic), and then the
    current_identifier are saved to the session
    """
    dss = default_subject_store
    ds = delegating_subject
    monkeypatch.setitem(ds.session._delegate.session, dss.dsc_isk, 'old_key')
    monkeypatch.setattr(ds, 'get_run_as_identifier_stack', lambda: True)
    with mock.patch.object(MockSession, 'set_attribute') as mock_sa:
        mock_sa.return_value = None
        dss.merge_identifier(ds)
        mock_sa.assert_called_once_with(dss.dsc_isk, ds._identifier)


def test_dss_merge_identifier_notrunas_withsession(
        default_subject_store, delegating_subject, monkeypatch):
    """
    unit tested:  merge_identifier

    test case:
    with run_as NOT set for the DS, current_identifier is obtained from
    the identifier property, and then the current_identifier are saved to the
    session
    """
    dss = default_subject_store
    ds = delegating_subject
    monkeypatch.setitem(ds.session._delegate.session, dss.dsc_isk, 'old_key')
    monkeypatch.setattr(ds, 'get_run_as_identifier_stack', lambda: False)
    with mock.patch.object(MockSession, 'set_attribute') as mock_sa:
        mock_sa.return_value = None
        dss.merge_identifier(ds)
        mock_sa.assert_called_once_with(dss.dsc_isk, ds.identifier)


def test_dss_merge_identifier_notrunas_withoutsession(
...
def test_dsf_create_subject(
    ...
    default_subject_context.identifier = simple_identifier_collection
