...
class UsernamePasswordToken(authc_abcs.AuthenticationToken):
    ...
    def credentials(self, credentials):
        if isinstance(credentials, bytearray):
            self._credentials = credentials
        if isinstance(credentials, str):
            self._credentials = bytearray(credentials, 'utf-8')
        else:
            raise InvalidTokenException('Password must be a str or bytearray')
...
class TOTPToken(AuthenticationToken):
...
class DefaultAuthenticator(authc_abcs.Authenticator, event_abcs.EventBusAware):
    ...
    def do_authenticate_account(self, authc_token):
        ...
        if account.authc_info['factors'] > authc_token.TIER:
